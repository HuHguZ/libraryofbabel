...
```
 ...
 └────────┘    └───────┘    └───────┘    └────────┘    └─────────┘
  ...
  каждая страница — 4 819 символов из 59-символьного алфавита:
  33 строчные русские буквы, 10 цифр, пробел
  и 15 знаков препинания  . , ! ? : ; - — … ( ) « » " '
```

Различных страниц в русской Библиотеке &mdash; 59<sup>4819</sup> ≈ 5,4 × 10<sup>8533</sup>. У Борхеса алфавит был скромнее: 22 буквы, пробел, запятая и точка.

Адрес галереи &mdash; строка из цифр и латинских букв обоих регистров (62 «цифры», по одной на каждый символ страницы). Любой адрес ведёт в свою галерею, и в ней всегда стоят одни и те же книги. Галереи и полки на скриншотах выше &mdash; из галереи с адресом `babel`.
...
| Буквы | 33 строчные русские | 26 строчных латинских |
| Цифры, пробел | 10 и пробел | 10 и пробел |
| Знаки препинания | `. , ! ? : ; - — … ( ) « » " '` | `. , ! ? : ; - — … ( ) " '` |
| Символов всего | 59 | 50 |
| Различных страниц | 59<sup>4819</sup> ≈ 5,4 × 10<sup>8533</sup> | 50<sup>4819</sup> ≈ 2,2 × 10<sup>8187</sup> |
...
Искать можно целые предложения со знаками препинания и числами. Заглавные буквы становятся строчными, типографские варианты знаков приводятся к алфавиту (`“ ”` → `"`, `–` → `—`), а символы, которых в Библиотеке нет, отбрасываются. Строка поиска заранее показывает, что именно будет искаться.
...
- **Обратимый алгоритм.** Каждый символ страницы &mdash; одна «цифра» адреса, сдвинутая на псевдослучайную величину из ключевого потока, который зависит от стены, полки, тома и страницы. Сдвиги берутся по модулю размера алфавита, поэтому одна и та же схема адресов обслуживает и 59-символьную русскую, и 50-символьную английскую Библиотеку: поиск записывает текст в адрес, чтение возвращает его обратно (`src/lib/babel.ts`, алфавиты &mdash; `src/lib/alphabet.ts`).
