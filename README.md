...
<p align="center">
  <img src="docs/screenshots/doorway.jpg" alt="Шестигранная галерея: у перил стоят читальные столы, а сквозь дверной проём видны винтовая лестница вестибюля и полки соседней галереи" width="100%" />
</p>
...
У перил шахты, напротив каждой из пяти стен с полками, стоит читальный стол с бронзовой лампой &mdash; на него ложится том, снятый с этой стены.
...
<table>
  <tr>
    <td width="50%">
      <img src="docs/screenshots/gallery.jpg" alt="Пять стен книжных полок вокруг вентиляционной шахты, у перил — читальные столы с лампами" />
      <br><sub><b>Шестигранная галерея.</b> Пять стен полок, лампа под потолком и читальные столы у перил шахты.</sub>
    </td>
    <td width="50%">
      <img src="docs/screenshots/aim.jpg" alt="Прицел в центре экрана наведён на том, подсказка показывает стену, полку и том" />
      <br><sub><b>Как в шутере.</b> Мышь ведёт взгляд, прицел выдвигает том, щелчок отправляет его на стол.</sub>
    </td>
  </tr>
  <tr>
    <td width="50%">
    ...
    <td width="50%">
      <img src="docs/screenshots/shaft.jpg" alt="Взгляд сверху через перила в шахту, вокруг перил читальные столы, внизу полки нижней галереи" />
      <br><sub><b>Шахта.</b> Столы стоят по кругу у перил, а под ними видны галереи этажом ниже &mdash; и так без конца.</sub>
    </td>
  </tr>
</table>
...
Нажмите на том &mdash; он выскользнет с полки, перелетит через галерею и опустится на стол у шахты, раскрывшись на титульном листе и указателе страниц. Кнопка «назад» &mdash; в шапке или в браузере &mdash; закрывает книгу и отправляет её обратно на полку, а взгляд возвращается туда, где вы стояли. Перелёт можно пропустить щелчком, пробелом, `Enter` или `Esc`; передумаете посередине пути &mdash; он пойдёт в обратную сторону.

<p align="center">
  <img src="docs/screenshots/take.webp" alt="Том слетает с полки, летит через галерею, ложится на стол и раскрывается; «назад» — книга закрывается и возвращается на своё место" width="100%" />
</p>
...
Разглядеть страницу помогает лупа: кнопка «лупа», клавиша `L` или нажатие колеса мыши. Она ходит за курсором и увеличивает всё, что под ней, &mdash; буквы, закладку, лампу, стол, а колесо меняет увеличение от ×1,5 до ×6. `Esc` убирает её.

<p align="center">
  <img src="docs/screenshots/loupe.webp" alt="Лупа появляется над страницей, ведёт по строкам, приближает текст сильнее, проходит над закладкой и лампой и исчезает" width="100%" />
</p>
...
<table>
  <tr>
    ...
    <td width="50%">
      <img src="docs/screenshots/volume.jpg" alt="Раскрытый том на столе у перил: титульный лист и указатель страниц, вокруг — галерея" />
      <br><sub><b>Раскрытый том.</b> Титульный лист и указатель на все 421 страницу &mdash; на столе у перил.</sub>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <img src="docs/screenshots/reader.jpg" alt="Разворот книги с текстом при свете настольной лампы, за столом — перила и полки галереи" />
      <br><sub><b>Читальный стол.</b> Разворот при свете настольной лампы и красная закладка, а вокруг &mdash; всё та же галерея.</sub>
    </td>
    <td width="50%">
...
**3D-прогулка** &mdash; бесконечный мир галерей от первого лица: полки с заглавиями, том, что летит на стол и раскрывается на нём, и читалка с перелистыванием &mdash; всё без смены экрана, на одной и той же сцене.
...
| Галерея, полка, том, читалка | пропустить перелёт | щелчок по сцене, пробел, `Enter` или `Esc` |
...
| | взять лупу / убрать | кнопка «лупа», `L` или нажатие колеса мыши / `Esc` |
| | увеличение лупы | колесо мыши, пока лупа в руке |
...
- **Одна сцена на всё.** Галерея, полка крупным планом, раскрытый том и читалка &mdash; не четыре страницы, а четыре состояния одной сцены: canvas живёт в общем макете маршрутов, а между состояниями «режиссёр» проигрывает перелёты по заранее посчитанным дорожкам (`src/components/explore/stage`). Поэтому книга действительно летит с полки на стол, а не подменяется новым экраном; при этом каждое состояние остаётся обычной ссылкой, которую можно открыть напрямую, а «назад» посреди перелёта разворачивает его с того же места.
...
- **Лупа** (`src/components/explore/loupe`) не растягивает готовый кадр, а рисует сцену ещё раз: та же камера, но её кадр обрезан (`setViewOffset`) до квадрата в N раз меньше линзы, и эта картинка ложится на стекло. Поэтому буквы под лупой остаются чёткими, пока хватает разрешения текстуры страницы. Шейдер стекла сжимает изображение к краю, чуть разводит цвета у оправы и добавляет блики. Лупа годится для любой 3D-сцены: `<Loupe active />` внутри `Canvas` и хук `useLoupe()` на странице. Модель &mdash; [Magnifying Glass 01](https://polyhaven.com/a/magnifying_glass_01), автор Nazar Borodavka, Poly Haven (CC0).
...
- **Хуки для отладки.** В режиме разработки открыты `window.__reader` &mdash; заглянуть в листы книги, кеш текстур страниц и вид, замедлить перелистывание &mdash; и `window.__stage`: `setTimeScale(scale)` замедляет или останавливает перелёт; `seek(fraction)` ставит его на заданную долю пути, от 0 до 1; `skip()` доигрывает его до конца; `state()` возвращает контроллер сцены и, если перелёт идёт, его вид и сколько секунд из скольких прошло; `hide(wall, shelf, volume)` убирает один том с полки, как будто его взяли; а `lightDesk(wall)` вручную зажигает лампу стола нужной стены, даже когда книги на нём нет (`null` гасит её).
